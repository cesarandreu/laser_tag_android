	...
	bluetoothPugin.fetchUUIDs(
	  function(uuids) {
        ...
        if (uuids.length == 0) {
          ...
          //Update text
          document.getElementById("connecting-uuid").innerHTML = uuids[0];
          document.getElementById("listening-uuid").innerHTML = uuids[0];
...
function connect() {
  ...
  var uuidDropdown = document.getElementById("uuid-list");
  listeningUUID = uuidDropdown.options[uuidDropdown.selectedIndex].text;
...
function toggleListen(item) {
  ...
  //alert('ListeningUUID: ' + listeningUUID);

  bluetoothPugin.read(
    function(result) {
      console.log('Result: ' + result);
      document.getElementById("listen-textarea").value += "Data : >" + result + "<\n";
    },
    function( error ) {
...
function doSomething(){
  bluetoothPlugin.read(function(answer) {
...
myObj = eval("{type: 'response', response: 'new'}");
*/